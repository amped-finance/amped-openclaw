 ...
 * for all Amped OpenClaw operations.
 */

/**
 * Standard error codes for Amped OpenClaw operations
 */
...
/**
 * Amped OpenClaw Error class
 */
export class AmpedOpenClawError extends Error {
  ...
  constructor(
    ...
    if (Error.captureStackTrace) {
      Error.captureStackTrace(this, AmpedOpenClawError);
    }
  }
...
export function createPolicyError(
  ...
  return new AmpedOpenClawError(code, message, {
...
/**
 * Wrap an unknown error into an AmpedOpenClawError
 */
export function wrapError(
  ...
  if (error instanceof AmpedOpenClawError) {
    return error;
  }
  ...
  if (error instanceof Error) {
    ...
    return new AmpedOpenClawError(code, error.message, {
      severity: ErrorSeverity.ERROR,
      context,
      cause: error,
    });
  ...
  return new AmpedOpenClawError(fallbackCode, String(error), {
    severity: ErrorSeverity.ERROR,
    context,
  });
}
 ...
 */
export function logError(error: AmpedOpenClawError | Error, context?: ErrorContext): void {
  const structuredLog = {
    timestamp: new Date().toISOString(),
    component: 'amped-openclaw',
    level: error instanceof AmpedOpenClawError ? error.severity : 'error',
    code: error instanceof AmpedOpenClawError ? error.code : ErrorCode.UNKNOWN_ERROR,
 ...
 */
export function isRetryableError(error: AmpedOpenClawError | Error): boolean {
  if (error instanceof AmpedOpenClawError) {
