...
export class PolicyEngine {
  ...
  private loadPolicyConfig(policyId?: string): PolicyConfig {
    ...
    if (!limitsJson) {
      console.warn('[policyEngine] AMPED_OC_LIMITS_JSON not set, using empty policy');
      return {};
    }
    ...
    try {
      ...
      if (policyId && !config) {
        console.warn(`[policyEngine] Policy profile not found: ${policyId}, using default`);
        return allConfigs['default'] || {};
      }

      return config || {};
    } catch (error) {
      console.error('[policyEngine] Failed to parse AMPED_OC_LIMITS_JSON', error);
      return {};
    }
  }
  ...
  async checkBridge(operation: BridgeOperation): Promise<PolicyCheckResult> {
    ...
    console.log('[policyEngine] Bridge operation passed all policy checks', {
      srcChainId,
      dstChainId,
      srcToken,
      dstToken,
  ...
  private checkSwapAmount(inputAmount: string, srcToken: string): PolicyCheckResult {
    ...
    // For now, we log the configured limit but don't enforce without prices
    if (maxSwapInputUsd) {
      console.log('[policyEngine] maxSwapInputUsd configured but requires price oracle for enforcement:', maxSwapInputUsd);
    }
  ...
  async checkSwap(params: {
    ...
    console.log('[policyEngine] Swap operation passed all policy checks', {
      srcChainId,
      dstChainId,
      srcToken,
      dstToken,
  ...
  async checkMoneyMarket(params: {
    ...
    console.log('[policyEngine] Money market operation passed all policy checks', {
      chainId,
      dstChainId,
      token,
      amount,
